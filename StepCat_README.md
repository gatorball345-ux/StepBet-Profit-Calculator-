...
For a Non-Member Game, StepBet takes 15% of the pot before payout is split.

```text
Payout = (Total Pot / Projected Winners) × 0.85
```

### Member Game

For a Member Game, the full pot is split among winners with no 15% deduction.

```text
Payout = Total Pot / Projected Winners
```

The selected game type is saved as a preference and remains selected after reopening the app.

---

## Projected Winners

**Projected Winners** means the estimated number of participants expected to receive a payout.

This is useful when checking possible profits while a game is still ongoing.

For example, if you think 10 people will finish the game successfully, enter:
...
Because History is persistent, the app asks for confirmation before clearing it. The confirmation message includes the number of saved entries being cleared.

---

## Result Labels

Each result card shows one of three labels:

- **Profit**
- **Draw**
- **Loss**

The label is based on the profit value:

- Positive profit = Profit
- Zero profit = Draw
- Negative profit = Loss

The result label is color-coded inside the app:
...
2. Choose the correct game type.
3. Tap **Calculate**.
4. Repeat for as many games as needed.
5. Tap **Copy Full History**.
...
3. Choose the correct game type.
4. Tap **Calculate**.
5. Tap **Copy Latest Row**.
6. Open the existing Google Sheet.
7. Select the next empty row.
8. Paste.
...
- Projected Winners is missing
- Any required value is not a number
- Projected Winners is zero
- Entry Fee is zero
...
### What are Projected Winners?

Projected Winners is the estimated number of participants expected to receive a payout.

The app divides the Total Pot by Projected Winners to estimate payout.

---

### What is the difference between Member Game and Non-Member Game?

A **Non-Member Game** deducts 15% from the pot before payout is split.

A **Member Game** splits the full pot among winners with no 15% deduction.
